...
public class FlickrConfiguration {
    ...
    @Value("${flickr.apiSecrect}")
    private String apiSecrect;

    @Value("${flickr.appKey}")
    private String appKey;
    @Value("${flickr.appSecrect}")
    private String appSecrect;

    //This method is used to generate flickr app keys
   /* @Bean
    public Flickr getFlickr() throws IOException, ExecutionException, InterruptedException, FlickrException {

        Flickr flickr = new Flickr(apiKey,apiSecrect, new REST());
        OAuth10aService service= new ServiceBuilder(apiKey)
                .apiSecret(apiSecrect)
    ...
    public Flickr getFlickr(){
        ...
        auth.setTokenSecret(appSecrect);
